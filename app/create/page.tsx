...
import { uploadImage, getSessionImages, saveToCart, getCartItems, deleteImage, uploadUserImage } from "@/services/cloudinary-service"
...
// Thay đổi export default function CreatePage() thành một wrapper component
export default function CreatePageWrapper() {
  return (
    <Suspense
      fallback={
        <div className="container mx-auto py-8 px-4">
          <h1 className="text-3xl font-bold mb-8">Tạo khung ảnh</h1>
...
// Chuyển component chính thành một component riêng biệt
function CreatePage() {
    ...
    const fetchTemplates = async () => {
      ...
      try {
        ...
        console.log(
          ...
          }ms, số lượng: ${templatesFromDb.length}`,
        )
        setTemplates(templatesFromDb)
        setFilteredTemplates(templatesFromDb)
  ...
  // Load ảnh đã tải lên từ Cloudinary khi phiên làm việc được khởi tạo
  useEffect(() => {
    const loadSessionImages = async () => {
      ...
      try {
        ...
        if (images.length > 0) {
          // Chuyển đổi ảnh từ Cloudinary thành UploadedImage
          const uploadedImgs: UploadedImage[] = images.map((img) => ({
            id: `image-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
            file: new File([], img.originalFilename), // Tạo file rỗng vì chúng ta đã có URL
            url: img.url,
            position: { x: 0, y: 0 },
            scale: 1,
            rotation: 0,
          ...
          setStoredImageIds(images.map((img) => img.id))

          toast({
            title: "Đã tải ảnh",
            description: `Đã tải ${images.length} ảnh từ phiên làm việc trước`,
          })
        }
      } catch (error) {
        ...
        toast({
          title: "Lỗi tải ảnh",
          description: "Đã xảy ra lỗi khi tải ảnh từ phiên làm việc trước",
          variant: "destructive",
        })
      } finally {
  ...
  const handleTemplateSelect = (template: Template) => {
    ...
    // Reset image slot assignments if template changes
    ...
    // Move to edit tab after selecting template
    if (uploadedImages.length > 0) {
      setActiveTab("edit")
    } else {
      // Nếu chưa có ảnh nào được tải lên, vẫn chuyển sang tab edit
      // nhưng hiển thị thông báo cho người dùng tải ảnh lên
      setActiveTab("edit")
      toast({
  ...
  // Lấy giá hiện tại dựa trên tùy chọn đã chọn
  const getCurrentPrice = (template: Template, option: string): number => {
    if (option === "download") {
      return template.download_sale_price || template.download_price
    } else {
      return template.print_sale_price || template.print_price
    }
  }

  // Trong phần handleAddToCart, cập nhật để sử dụng giá dựa trên tùy chọn đã chọn
  const handleAddToCart = async () => {
    if (!officialUrl || !selectedTemplate || !sessionId) return

    setIsSavingToCart(true)

    try {
      // Lấy danh sách ID của các ảnh đã được sử dụng trong các slot
      const usedImageIds = uploadedImages.filter((img) => img.slotId && img.publicId).map((img) => img.id)

      // Lấy giá dựa trên tùy chọn đã chọn
      const price = getCurrentPrice(selectedTemplate, selectedPriceOption)

      // Lưu vào giỏ hàng với giá từ template và tùy chọn đã chọn
      await saveToCart(
        officialUrl,
        selectedTemplate.id,
        usedImageIds,
        price,
        sessionId,
        selectedPriceOption, // Thêm loại sản phẩm vào thông tin giỏ hàng
        selectedTemplate.name, // Thêm tên template vào thông tin giỏ hàng
      )
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Button onClick={handleAddToCart} disabled={isSavingToCart}>
                {isSavingToCart ? (
                  <>
                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
                    Đang xử lý...
      ...
      <div className="container mx-auto py-8 px-4">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="template">Chọn mẫu</TabsTrigger>
            <TabsTrigger value="edit" disabled={!selectedTemplate}>
              Chỉnh sửa
            </TabsTrigger>
            <TabsTrigger value="preview" disabled={!selectedTemplate || !allSlotsHaveImages()}>
              Xem trước
            </TabsTrigger>
          </TabsList>
